...
Test hypotheses:
```{r}
postprob = function(model, term) {
 library(brms); test = hypothesis(model[[1]], term, alpha = 0.1)
 term = test$hypothesis[1]; postprob = test$hypothesis[7]; out = cbind(term, postprob)
 return(out)
}

hyptests = rbind(
 cbind(m = "m1a", postprob(m1a, "prof_tradMedium<0")), 
 cbind(m = "m1a", postprob(m1a, "prof_tradHigh<0")), 
 cbind(m = "m1a", postprob(m1a, "prof_tradMedium - prof_tradHigh > 0")), 
 cbind(m = "m1b", postprob(m1b, "partradMedium:lr_bright > 0 ")), 
 cbind(m = "m1b", postprob(m1b, "partradHigh:lr_bright>0")), 
 cbind(m = "m1b", postprob(m1b, "partradHigh:lr_bright - partradMedium:lr_bright >0")),
 cbind(m = "m2a", postprob(m2a, "autYes>0")), 
 cbind(m = "m2b", postprob(m2b, "autYes:lr_bright<0")))
